...
class KeycloakSyncMixin(models.AbstractModel):
    ...
    login_match_key = fields.Selection(
        ...
        default='email:partner_id.email',
    )
...
class KeycloakSyncWiz(models.TransientModel):
    ...
    def button_sync(self):
        ...
        # create user if does not exist in odoo
        for user in users:
            user_id = self.env['res.users'].search([('email', '=', user.get('email'))])
            if not user_id:
                self.env['res.users'].create({
                    'name': '%s %s' % (user.get('firstName'), user.get('lastName')),
...
class KeycloakCreateWiz(models.TransientModel):
    ...
    def button_create_user(self):
        ...
        for user in self.user_ids:
            # Delete any existing OAuth Access Token
            user.oauth_access_token = False
            if user.oauth_uid:
